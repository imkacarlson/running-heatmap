...
This process creates a native Android app that bundles all your run data locally, allowing it to work without an internet connection.
...
*   **`runs.pkl` file**: The data file generated by `process_data.py`.
...
2.  **Run the build script:**
   ...
   The build process automatically includes vector tiles for faster map rendering when available.
...
3.  ✅ **Build Mobile Assets**: Convert your `runs.pkl` data into a mobile-friendly format.
...
Running `python process_data.py` (from the `server` directory) will import GPS data
and create `runs.pmtiles` for faster map loading. This consolidated script handles
both data import and PMTiles generation in one step.
...
python process_data.py                                  # whenever new raw files are added
```
...
```
running-heatmap/
├── server/              # Data processing and mobile build tools
│   ├── process_data.py # Consolidated GPS processing script
│   ├── app.py          # API server for mobile app uploads
│   ├── runs.pkl        # Your processed run data
│   └── build_mobile.py # All-in-one mobile build script
├── mobile/             # Generated Capacitor Android project
│   ├── www/            # Mobile app assets and bundled data
│   ├── android/        # Native Android project
│   ├── node_modules/   # Dependencies
│   └── running-heatmap-*.apk # Your installable APK
...
*   ✅ **All data stays local** - no cloud services or internet connection required for the mobile app.
